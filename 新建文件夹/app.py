...
def sync_lcu_matches_impl(user_id, season_filter=None, source="auto", platform=None):
    ...
    for raw_game in games:
        ...
        # 保存这局所有玩家的信息（不管对局是否已存在，都补存对手数据）
        c.execute("SELECT COUNT(*) FROM match_players WHERE game_id=?", (game_id,))
        has_players = c.fetchone()[0] > 0
        if not has_players:
            ...
            for p in all_participants:
                ...
                if not p_puuid:
                    pid = p.get("participantId")
                    p_puuid = pid_to_puuid.get(pid, "")
                p_name = name_map.get(p_puuid, "") or _get_p_stat(p, "summonerName") or _get_p_stat(p, "gameName") or ""
